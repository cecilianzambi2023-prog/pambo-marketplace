...
export const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({
  ...
  const commentsForListing = (listingComments[product.id] || []).filter(
            ...
            {product.gallery && product.gallery.length > 0 && (
              <div className="p-2 flex gap-2 overflow-x-auto bg-gray-900 scrollbar-hide">
                ...
                {product.gallery.map((img, i) => (
                  <div
                    key={i}
                    className="w-16 h-16 rounded border border-gray-600 overflow-hidden shrink-0 cursor-pointer hover:border-white transition"
                  >
                    <SmartImage
                      src={img}
          ...
          <div className="md:w-1/2 p-6 md:p-8 flex flex-col overflow-y-auto">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-100 space-y-2">
              <div className="text-sm text-gray-700">
                Payment Arrangement:{' '}
                <span className="font-semibold">{paymentArrangementLabel}</span>
              </div>
              {product.seller_phone && (
                <div className="text-sm text-gray-700 flex items-center gap-2">
                  <Phone size={14} /> Seller Number:{' '}
                  <span className="font-semibold">{product.seller_phone}</span>
                </div>
              )}
            </div>
            ...
            <div className="border-t border-gray-100 pt-6">
              ...
              {isLoggedIn ? (
                <form onSubmit={handleReviewSubmit} className="mb-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <button
                    type="submit"
                    className="bg-orange-600 text-white font-semibold text-sm py-2 px-4 rounded-md mt-2 hover:bg-orange-700 flex items-center gap-2"
                  >
            ...
            <div className="border-t border-gray-100 pt-6 mt-6">
              <h3 className="text-gray-900 font-bold mb-4">
                Comments ({commentsForListing.length})
              </h3>
              {isLoggedIn ? (
                <form onSubmit={handleCommentSubmit} className="mb-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <button
                    type="submit"
                    className="bg-gray-900 text-white font-semibold text-sm py-2 px-4 rounded-md mt-2 hover:bg-black flex items-center gap-2"
                  >
                ...
                {commentsForListing.length > 0 ? (
                  commentsForListing.map((comment, index) => (
                    <div
                      ...
                      className="text-sm bg-gray-50 rounded-md p-3 border border-gray-100"
                    >
                      <div className="flex items-center justify-between">
