...
export const AiAssistantWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      sender: 'ai',
      text: "Hello! I'm the Pambo Assistant. How can I help you with buying, selling, or our services today?"
    }
  ]);
  ...
  return (
    <>
      {/* Chat Widget Window */}
      <div
        className={`fixed bottom-24 right-4 md:bottom-6 md:right-6 w-80 md:w-96 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 transition-all duration-300 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
      >
        {/* Header */}
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              ...
              >
                ...
                <div
                  className={`max-w-[80%] p-3 rounded-2xl text-sm ${msg.sender === 'user' ? 'bg-orange-500 text-white rounded-br-none' : 'bg-white text-gray-800 rounded-bl-none border border-gray-200'}`}
                >
                  {msg.text}
                </div>
              </div>
        ...
        <div className="p-3 border-t border-gray-200">
          <div className="relative">
            ...
            <button
              onClick={handleSend}
              disabled={isLoading}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-orange-500 text-white rounded-md hover:bg-orange-600 disabled:bg-gray-300"
            >
              <Send size={16} />
            </button>
          </div>
