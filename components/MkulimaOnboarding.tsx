...
import { Leaf, Phone, Loader2, CheckCircle, Globe, Users, ShieldCheck, Zap, Share2 } from 'lucide-react';
import { FarmerProfile } from '../types';
import { initiateSTKPush, formatPhoneForMPesa } from '../services/mpesaService';

interface MkulimaSignupProps {
    onJoin: (farmerDetails: { name: string; phone: string; location: string; crop: string; subscriptionExpiry: number; coordinates: { lat: number; lng: number; }; county: string; }) => void;
}
...
export const MkulimaSignup: React.FC<MkulimaSignupProps> = ({ onJoin }) => {
  ...
  const handleFarmerJoin = async () => {
    ...
    try {
        ...
        console.warn("M-Pesa API call failed, likely due to placeholder credentials. Simulating success for demo.", err);
  ...
  return (
    <div className="p-6 md:p-8 bg-white rounded-xl border border-gray-200 shadow-lg max-w-lg mx-auto">
        <div className="text-center">
            ...
            <p className="text-green-700 mb-6">Join for KES {SUBSCRIPTION_FEE}/year and we'll find customers for your produce!</p>
        </div>

        {step === 'input' && (
            <div className="space-y-4 animate-fade-in">
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Your M-Pesa Number</label>
                    <div className="relative">
                        <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                        <input
                            type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="e.g. 0712345678"
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Main Crop You Sell</label>
                    <select value={crop} onChange={(e) => setCrop(e.target.value as any)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-md focus:border-green-500 outline-none bg-white">
                ...
                <button onClick={handleFarmerJoin} className="w-full mt-4 bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 transition shadow-lg">
        ...
        {step === 'success' && (
            <div className="text-center py-8 animate-fade-in">
                ...
                <p className="text-sm text-gray-600">
                    Welcome to Pambo Mkulima! Your subscription is active for 1 year. Our AI is now working to find clients for your {crop}.
                </p>
            </div>
        )}
    </div>
...
export const MkulimaDashboard: React.FC<{ farmerName: string; crop: string }> = ({ farmerName, crop }) => {
  const stats = {
    ...
    <div className="max-w-md mx-auto bg-white min-h-screen pb-20">
      ...
      <div className="p-4 -mt-6 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-md border-t-4 border-green-500 text-center">
          ...
          <h3 className="text-4xl font-black text-gray-800">{stats.marketReach.toLocaleString()}</h3>
          <p className="text-gray-600 font-bold text-xs uppercase tracking-tight">Market Reach (People Reached)</p>
        ...
        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
           <p className="text-[9px] text-gray-400 leading-tight italic">
             Disclaimer: Offspring Decor Ltd provides marketing visibility. We connect you to the market, but final sales depend on your produce quality and price agreement with buyers.
           </p>
        </div>
