...
// Free limit: 4 images per listing. At 5+ images, payment required
const PHOTO_LIMIT_FREE = 4;
const PHOTO_LIMIT_PAID = 20; // With subscription
const VIDEO_LIMIT = 2;
...
export const AddListingModal: React.FC<AddListingModalProps> = ({
  ...
  const [paymentArrangement, setPaymentArrangement] =
    useState<Product['paymentArrangement']>('jiji_direct');
  const [showPaymentModal, setShowPaymentModal] = useState(false);
  const [paymentTriggerImageCount, setPaymentTriggerImageCount] = useState(0);
  ...
  useEffect(() => {
    if (isOpen) {
      if (productToEdit) {
        ...
        setIsDigital(
          productToEdit.isDigital || DIGITAL_CATEGORIES.includes(productToEdit.category)
        );
        ...
        const selectedCategory =
          initialCategory && categoryList.includes(initialCategory)
            ? initialCategory
            : defaultCategory;
  ...
  const handleFiles = (files: FileList | null | File[], type: 'cover' | 'gallery' | 'video') => {
    ...
    if (type === 'cover') {
      ...
      // Free tier: max 4 images. At 5+, show payment options
      const totalImages = gallery.length + filesArray.length + (image ? 1 : 0);

      if (totalImages > PHOTO_LIMIT_FREE && totalImages <= PHOTO_LIMIT_PAID) {
        // User trying to add 5+ images - show payment modal
        setPaymentTriggerImageCount(totalImages);
        setShowPaymentModal(true);
        alert(
          `You're adding the ${totalImages}th image! Subscribe to add more than ${PHOTO_LIMIT_FREE} images per listing. Choose a plan below.`
        );
        return;
      }

      if (totalImages > PHOTO_LIMIT_PAID) {
        alert(
          `Maximum ${PHOTO_LIMIT_PAID} images allowed. You are trying to add ${totalImages} total images.`
        );
        return;
      }
  ...
  const handleTypeChange = (type: 'product' | 'service') => {
    setListingType(type);
    const newCategory =
      type === 'product' ? DETAILED_PRODUCT_CATEGORIES[0].subcategories[0] : SERVICE_CATEGORIES[0];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // VALIDATION GATES - Production Quality
    if (!title || !image || (listingType === 'product' && !price)) {
      alert('❌ Please fill in all required fields: title, photo, and price.');
      return;
    }

    // Validate title length (min 10 chars for quality)
    if (title.trim().length < 10) {
      alert('❌ Title too short. Please use at least 10 characters to describe your item clearly.');
      return;
    }

    // Validate description (min 20 chars for trust)
    if (description.trim().length < 20) {
      alert('❌ Please add a detailed description (at least 20 characters) so buyers know what they\'re getting.');
      return;
    }

    // Validate seller phone (required for Jiji-style contact)
    if (!resellerPhone || resellerPhone.trim().length < 10) {
      alert('❌ Please provide a valid phone number so buyers can contact you directly.');
      return;
    }

    // Validate price (block unrealistic values)
    if (listingType === 'product' && price) {
      const priceNum = parseFloat(price);
      if (priceNum < 10) {
        alert('❌ Price too low. Minimum price is KES 10.');
        return;
      }
      if (priceNum > 10000000) {
        alert('❌ Price too high. Maximum price is KES 10,000,000. Contact support for higher-value items.');
        return;
      }
    }

    // ✨ SUBSCRIPTION CHECK: Only paid members can post
    if (!productToEdit && sellerId) {
      // Only check for new listings
      const canPost = await canSellerPost(sellerId, sellerEmail);
      if (!canPost) {
        setSubscriptionError(
          '❌ Only paid subscribers can list items. Please choose a subscription plan to start selling.'
        );
        return;
      }
    }
    ...
    const productData: Product = {
      ...
      paymentArrangement:
        listingType === 'product' && !isWholesale && !isDigital ? paymentArrangement : undefined
    };
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 custom-scrollbar">
          ...
          <div className="space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Cover Photo (1 of {PHOTO_LIMIT_FREE})
              </label>
              <div
              ...
              >
                ...
                {image ? (
                  ...
                  <div className="text-center text-gray-500 pointer-events-none">
                    ...
                    <p className="text-xs">
                      <span className="font-semibold text-orange-600">Click to upload</span> or drag
                      and drop
                    </p>
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-800 flex items-center gap-2 mb-2">
                <ImageIcon size={16} /> Image Gallery ({(image ? 1 : 0) + gallery.length} of{' '}
                {PHOTO_LIMIT_FREE})
              </h3>

              {(image ? 1 : 0) + gallery.length >= PHOTO_LIMIT_FREE && (
                <div className="mb-3 p-3 bg-blue-50 border border-blue-300 rounded text-sm text-blue-800 flex items-start gap-2">
                  <AlertCircle size={18} className="flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-semibold">Need more images?</p>
                    <p className="text-xs mt-1">
                      Subscribe to add more than {PHOTO_LIMIT_FREE} images per listing. Get access
                      to {PHOTO_LIMIT_PAID}+ images with a monthly subscription.
                    </p>
                    <button
                      type="button"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {listingType === 'product' ? 'Product Name' : 'Service Title'}
            </label>
            <input
              required
              maxLength={70}
              ...
              placeholder={
                listingType === 'product'
                  ? 'e.g. Handmade Leather Bag'
                  : 'e.g. Professional Home Cleaning'
              }
              value={title}
          ...
          {listingType === 'product' && !isWholesale && !isDigital && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Payment Arrangement (Jiji Style)
              </label>
              <select
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-orange-500 outline-none"
                value={paymentArrangement}
                onChange={(e) =>
                  setPaymentArrangement(e.target.value as Product['paymentArrangement'])
                }
              >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Seller Phone Number <span className="text-red-500">*</span>
            </label>
            <input
              required
              type="tel"
              className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-orange-500 outline-none"
              placeholder="e.g. 0712345678 or 0701234567"
              value={resellerPhone}
            ...
            <p className="text-xs text-gray-400 mt-1">
              <strong>Required:</strong> This number is shown to buyers for direct WhatsApp/Call contact (Jiji-style).
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-right text-xs text-gray-400 mt-1">
              {description.length}/500 characters (min. 20 required)
            </p>
          </div>
          ...
          >
            {isModerating ? (
              <>
                <Loader2 size={18} className="animate-spin" /> Moderating...
              </>
            ) : (
              <>
                <Plus size={18} /> {productToEdit ? 'Save Changes' : `Post ${listingType}`}
              </>
            )}
          </button>
        </form>
