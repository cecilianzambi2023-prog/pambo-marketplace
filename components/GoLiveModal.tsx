...
export const GoLiveModal: React.FC<GoLiveModalProps> = ({ isOpen, onClose, sellerProducts, onStartStream, onViolation }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div className="bg-red-600 p-4 flex justify-between items-center text-white">
          <h2 className="text-lg font-bold flex items-center gap-2"><Wifi /> Go Live</h2>
          <button onClick={onClose} disabled={status === 'streaming'}><X size={20} /></button>
        </div>
        <div className="p-6">
            <div className="relative aspect-video bg-black rounded-lg overflow-hidden mb-4">
                ...
                {status === 'streaming' && (
                    <div className="absolute top-3 left-3 bg-red-600 text-white font-bold text-xs px-2 py-1 rounded animate-pulse">LIVE</div>
                )}
            </div>
             {status === 'setup' && (
                 <div className="space-y-4">
                     ...
                     >
                         ...
                         {sellerProducts.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
                     </select>
                     <button
                         onClick={handleStart}
            ...
            {status === 'streaming' && (
                <div className="text-center p-4 bg-green-50 rounded-lg border border-green-200">
                    ...
                    <p className="text-sm text-green-700">AI moderation is active to keep the stream safe.</p>
                </div>
            )}
