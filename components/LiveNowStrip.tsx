...
const LiveSellerAvatar: React.FC<{ stream: LiveStream; seller?: User; onClick: () => void }> = ({ stream, seller, onClick }) => {
    if (!seller) return null;
    return (
        <div onClick={onClick} className="flex flex-col items-center gap-2 cursor-pointer text-center w-20">
            <div className="relative">
                ...
                <img src={seller.avatar} alt={seller.name} className="w-16 h-16 rounded-full object-cover border-2 border-white relative" />
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 -mb-2">
                {liveStreams.map(stream => {
                    ...
                    return <LiveSellerAvatar key={stream.id} stream={stream} seller={seller} onClick={() => onViewStream(stream)} />;
                })}
            </div>
        </div>
    );
