...
export const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose, cartItems, onUpdateItem, onCheckout }) => {
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center mt-2">
                    <div className="flex items-center border border-gray-300 rounded">
                      <button onClick={() => onUpdateItem(item.id, item.quantity - 1)} className="px-2 py-1 text-sm">-</button>
                      <span className="px-3 text-sm">{item.quantity}</span>
                      <button onClick={() => onUpdateItem(item.id, item.quantity + 1)} className="px-2 py-1 text-sm">+</button>
                    </div>
                    <button onClick={() => onUpdateItem(item.id, 0)} className="ml-auto text-red-500 hover:text-red-700">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
