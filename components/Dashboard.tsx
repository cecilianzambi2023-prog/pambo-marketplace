...
import { Plus, Edit, Trash2, ShieldCheck, User as UserIcon, Calendar, Clock, AlertTriangle, Package, Briefcase, Lock, LogOut, Settings, List, EyeOff, Eye, CircleDotDashed, Check, X, Phone, ShoppingCart, Wifi, Users, Truck, CreditCard, Grid3X3 } from 'lucide-react';
...
type DashboardTab = 'listings' | 'orders' | 'sales' | 'followers' | 'subscription' | 'categories' | 'settings';
...
const ListingStatusBadge: React.FC<{ status: Product['listingStatus'] }> = ({ status }) => {
    ...
    return (
        <span className={`text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-md ${bgColor} text-white`}>
            {icon} {text}
        </span>
    );
};

const ProductStatusControl: React.FC<{ product: Product, onStatusChange: (productId: string, status: Product['status']) => void }> = ({ product, onStatusChange }) => {
    ...
    return (
        <div className="absolute top-2 right-2 z-10">
            <button onClick={() => setIsOpen(!isOpen)} className="bg-black/50 text-white p-1.5 rounded-full hover:bg-black/70">
...
const AccountSettings: React.FC<{ user: User, onDeleteAccountRequest: () => void, onUpdateProfile: (details: { phone?: string }) => void }> = ({ user, onDeleteAccountRequest, onUpdateProfile }) => {
     ...
     <div className="bg-white p-6 rounded-lg border border-gray-200 mt-6 animate-fade-in">
        ...
        <div className="space-y-4">
            <div>
                ...
                <p className="text-xs text-gray-500 mb-2">This number will be shown to buyers who want to contact you.</p>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
            <div className="p-4 border-2 border-red-300 bg-red-50 rounded-lg">
                ...
                <p className="text-sm text-red-600 mt-1 mb-4">
                    Request to permanently delete your account and all associated data. This action is irreversible and requires admin approval.
                </p>
                <button 
...
const MyOrders: React.FC<{ orders: Order[], onConfirmDelivery: (orderId: string) => void }> = ({ orders, onConfirmDelivery }) => {
    return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 mt-6 animate-fade-in">
        ...
        <div className="space-y-6">
            ...
            <div key={order.id} className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 p-3">
                    ...
                    <p className="text-xs text-gray-500">Date: {new Date(order.date).toLocaleDateString()}</p>
                </div>
                <div className="p-4">
                    <div className="mb-4">
                     ...
                     {order.status === 'Delivered' && (
                        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-center mb-4">
                           <p className="text-sm text-blue-800 mb-2">The seller has marked this item as delivered. Please confirm receipt to release payment.</p>
                           <button onClick={() => onConfirmDelivery(order.id)} className="bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700">
                     ...
                     {order.items.map(item => (
                        <div key={item.productId} className="flex items-center gap-4 py-2">
                            ...
                            <p className="text-sm font-semibold text-gray-800">KES {item.price.toLocaleString()}</p>
                        </div>
                    ))}
                </div>
                <div className="bg-gray-50 p-3 text-right">
                    <p className="text-md font-bold text-gray-900">Total: KES {order.totalAmount.toLocaleString()}</p>
                </div>
            </div>
            ))}
        </div>
...
const SalesManagement: React.FC<{ sales: Order[], onStatusChange: (orderId: string, status: Order['status']) => void }> = ({ sales, onStatusChange }) => {
                 ...
                 <div className="space-y-4">
                    {sales.map(sale => (
                        <div key={sale.id} className="border border-gray-200 rounded-lg p-4">
                            ...
                            {sale.items.map(item => (
                                <div key={item.productId} className="flex items-center gap-3 py-2 border-t border-gray-100">
                                    <img src={item.image} className="w-10 h-10 rounded object-cover" />
                                    <p className="text-sm flex-1">{item.title} (x{item.quantity})</p>
...
const FollowersManager: React.FC<{ followerIds: string[], allSellers: User[] }> = ({ followerIds, allSellers }) => {
    const followerDetails = (followerIds || []).map(id => allSellers.find(s => s.id === id)).filter(Boolean) as User[];

    return (
        <div className="bg-white p-6 rounded-lg border border-gray-200 mt-6 animate-fade-in">
            <h3 className="text-lg font-bold text-gray-800 mb-4">Your Followers ({followerDetails.length})</h3>
            {followerDetails.length === 0 ? (
                ...
                <div className="space-y-3">
                    {followerDetails.map(follower => (
                        <div key={follower.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Joined: {new Date(follower.joinDate).toLocaleDateString()}</p>
                                </div>
                            </div>
...
export const Dashboard: React.FC<DashboardProps> = ({ user, listings, orders, sales, isSubscriptionActive, onAddListing, onEditListing, onDeleteListing, onManageSubscription, onLogout, onDeleteAccountRequest, onProductStatusChange, onOrderStatusChange, onUpdateProfile, onGoLive, liveStreams, sellers }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">Pambo</h1>
                ...
                <div>
                 ...
                 <div className="flex items-center gap-2">
                    <button 
                        ...
                        title={!isSubscriptionActive ? "VVVIP Subscription Required for Live Commerce" : "Start a live stream"}
                    >
                        {!isSubscriptionActive && <Lock size={14} className="absolute -top-1 -right-1 text-gray-600" />}
                    ...
                    <button onClick={onLogout} className="bg-gray-200 text-gray-700 p-2 rounded-full hover:bg-gray-300 transition" title="Logout">
                        <LogOut size={16} />
                    </button>
                </div>
            </header>
            ...
            {showRenewalPrompt && user.role === 'seller' && user.accountStatus === 'active' && (
                <div className={`border-l-4 p-4 rounded-r-lg ${isSubscriptionExpiringSoon ? 'bg-red-50 border-red-500' : 'bg-blue-50 border-blue-500'}`}>
                    <p className={`text-sm font-semibold ${isSubscriptionExpiringSoon ? 'text-red-700' : 'text-blue-700'}`}>
                        {!isSubscriptionActive && !user.subscriptionExpiry && 'You are on the Free Plan.'}
                        {isSubscriptionExpiringSoon && daysRemaining > 0 && `⚠️ Your subscription renews in ${daysRemaining} day${daysRemaining > 1 ? 's' : ''}!`}
                        {isSubscriptionExpiringSoon && daysRemaining <= 0 && `⚠️ Your subscription renews TODAY!`}
                        {isSubscriptionActive && !isSubscriptionExpiringSoon && 'Your subscription has expired.'}
                        <a href="#" onClick={(e) => { e.preventDefault(); onManageSubscription(); }} className="font-bold underline ml-2 hover:opacity-80">
                            {isSubscriptionExpiringSoon ? 'Renew Now' : (user.subscriptionExpiry ? 'Renew Now' : 'Upgrade to Premium to post services & get premium benefits!')}
                        </a>
                    </p>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard title="Subscription" value={isSubscriptionActive ? 'VVVIP Plan' : 'Free Plan'} icon={<Calendar size={22} />} />
                {isSubscriptionActive ? (
                   <StatCard title="Days Remaining" value={daysRemaining} icon={<Clock size={22} />} />
                ) : (
                   <StatCard title="Free Products Used" value={`${freeProductListingsUsed} / ${FREE_LISTING_LIMIT}`} icon={<Package size={22} />} />
                )}
            </div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xl font-bold text-gray-800">My Listings</h2>
                         <div className="flex items-center gap-2">
                            ...
                            <button 
                                onClick={() => isSubscriptionActive ? onAddListing('service') : onManageSubscription()}
                                className={`font-bold py-2 px-4 rounded-lg transition flex items-center gap-2 shadow-sm relative ${
                                    isSubscriptionActive 
                                    ? 'bg-blue-600 text-white hover:bg-blue-700' 
                                    : 'bg-gray-200 text-gray-500 cursor-pointer hover:bg-gray-300'
                                }`}
                                title={!isSubscriptionActive ? "VVVIP Subscription Required" : "Post a new service ad"}
                            >
                                {!isSubscriptionActive && <Lock size={14} className="absolute -top-1 -right-1 text-gray-600" />}
            ...
            {activeTab === 'orders' && <MyOrders orders={orders} onConfirmDelivery={(orderId) => onOrderStatusChange(orderId, 'Completed')} />}
            {activeTab === 'sales' && <SalesManagement sales={sales} onStatusChange={onOrderStatusChange} />}
            {activeTab === 'followers' && <FollowersManager followerIds={user.followers || []} allSellers={sellers} />}
            ...
            {activeTab === 'subscription' && (
                <div className="bg-white rounded-lg border border-gray-200 p-6 animate-fade-in">
                    ...
                    <p className="text-gray-600 mb-8">Choose a plan to unlock more features and get access to premium hubs. 0% commission on all sales.</p>
                    <PricingGrid userId={user.id} />
                </div>
            )}
            {activeTab === 'settings' && (
                <AccountSettings user={user} onDeleteAccountRequest={onDeleteAccountRequest} onUpdateProfile={onUpdateProfile} />
            )}
        </div>
    );
};
