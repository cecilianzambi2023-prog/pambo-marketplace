...
export const FarmersMapView: React.FC<FarmersMapViewProps> = ({ farmers, onContactFarmer, onGetDeliveryInsight, insightLoadingFarmerId }) => {
    return (
        <div className="relative w-full h-96 md:h-[500px] rounded-lg overflow-hidden border-2 border-gray-200">
            <MapContainer center={[-1.286389, 36.817223]} zoom={7} scrollWheelZoom={false} className="w-full h-full">
                <TileLayer
                    attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                />
                {farmers.map(farmer => (
                    <Marker key={farmer.id} position={[farmer.coordinates.lat, farmer.coordinates.lng]} icon={defaultIcon}>
                        <Popup>
                            <div className="text-center p-1 w-52">
                                <img src={farmer.avatar} alt={farmer.name} className="w-14 h-14 rounded-full object-cover mx-auto mb-2 border-2 border-green-500" />
                                <h4 className="font-bold text-sm text-gray-800">{farmer.name}</h4>
                                <p className="text-xs text-gray-500">Selling: <strong>{farmer.crop}</strong></p>
                                <p className="text-xs text-gray-500">Price: <strong className="text-gray-800">KES {farmer.pricePerUnit}/{farmer.unit}</strong></p>
                                ...
                                <div className="mt-2 pt-2 border-t border-gray-100 text-left">
                                    {farmer.deliveryInsight ? (
                                        <div>
                                            ...
                                            <p className="text-[11px] text-gray-600 italic bg-gray-50 p-1.5 rounded">"{farmer.deliveryInsight}"</p>
                                        </div>
                                        ...
                                        <button
                                            onClick={(e) => { e.stopPropagation(); onGetDeliveryInsight(farmer.id); }}
