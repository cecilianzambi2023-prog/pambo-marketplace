...
const ChatMessage: React.FC<{ name: string; message: string; isHighlight?: boolean }> = ({ name, message, isHighlight }) => (
...
export const LiveStreamPlayer: React.FC<LiveStreamPlayerProps> = ({ stream, featuredProduct, onClose, onBuyNow, mediaStream }) => {
            ...
            <div className="relative w-full h-full max-w-6xl mx-auto flex flex-col md:flex-row bg-gray-900 shadow-2xl">
                ...
                <div className="w-full md:w-3/4 h-1/2 md:h-full bg-black flex items-center justify-center relative">
                    <video
                        ...
                        ref={videoRef}
                        src={!mediaStream ? "https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerFun.mp4" : undefined}
                        autoPlay
                        loop
                        muted
                        playsInline
                    ...
                    {isTerminated && (
                        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-center p-8 z-10">
                            ...
                            <button onClick={onClose} className="mt-6 bg-white text-black font-bold py-2 px-6 rounded-lg">
                                Close
                            </button>
                        </div>
                    )}
                    ...
                    {!isTerminated && featuredProduct && (
                        <div className="absolute bottom-4 left-4 right-4 z-10 bg-black/50 p-3 rounded-lg backdrop-blur-sm flex items-center gap-4 animate-fade-in-up">
                            <img src={featuredProduct.image} className="w-16 h-16 rounded-md object-cover" alt={featuredProduct.title} />
                            <div className="flex-1">
                                <h4 className="font-bold text-white line-clamp-1">{featuredProduct.title}</h4>
                                <p className="text-orange-400 font-bold text-lg">KES {featuredProduct.price?.toLocaleString()}</p>
                            </div>
                            <button onClick={handleBuyNow} className="bg-orange-600 text-white font-bold py-3 px-5 rounded-lg hover:bg-orange-700 transition flex items-center gap-2 shrink-0">
                ...
                <div className="w-full md:w-1/4 h-1/2 md:h-full bg-white flex flex-col">
                    <div className="p-4 border-b border-gray-200">
                        <div className="flex items-center gap-3">
                            <img src={stream.sellerAvatar} alt={stream.sellerName} className="w-10 h-10 rounded-full object-cover" />
                            <div>
